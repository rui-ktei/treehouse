...
func addGetFlags(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVar(&getJSON, "json", false, "Print lease allocation as JSON (requires --lease)")
}
...
func getLeaseRunE(repoRoot, poolDir string, cfg config.Config) error {
	...
	lease, err := pool.AcquireLeaseInfo(repoRoot, poolDir, cfg.MaxTrees, cfg.Hooks.PostCreate, holder, getBranch, cfg.SyncIgnored)
	if err != nil {
		return err
	}
