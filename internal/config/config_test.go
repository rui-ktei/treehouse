...
func TestLoad_SyncIgnoredHonorsUserLevelOverrideWhenRepoConfigOmitsKey(t *testing.T) {
	repoDir := t.TempDir()
	userHome := t.TempDir()
	setUserHome(t, userHome)

	repoTOML := `max_trees = 8`
	if err := os.WriteFile(filepath.Join(repoDir, "treehouse.toml"), []byte(repoTOML), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	userTOML := `sync_ignored = [".env.local"]`
	if err := os.WriteFile(filepath.Join(configDir, "config.toml"), []byte(userTOML), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestLoad_SyncIgnoredRepoOverrideWinsOverUserWhenBothSet(t *testing.T) {
	repoDir := t.TempDir()
	userHome := t.TempDir()
	setUserHome(t, userHome)

	repoTOML := `sync_ignored = ["appsettings*.local.json"]`
	if err := os.WriteFile(filepath.Join(repoDir, "treehouse.toml"), []byte(repoTOML), 0o644); err != nil {
		t.Fatal(err)
	}
